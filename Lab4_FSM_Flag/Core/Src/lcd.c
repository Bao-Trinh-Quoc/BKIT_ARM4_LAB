...
void lcd_draw_gradient(uint16_t topColor, uint16_t bottomColor)
{
    for (uint16_t y = 0; y < lcddev.height; y++) {
        float ratio = (float)y / lcddev.height;

        uint16_t r = ((1.0f - ratio) * ((topColor >> 11) & 0x1F) +
                       ratio * ((bottomColor >> 11) & 0x1F));
        uint16_t g = ((1.0f - ratio) * ((topColor >> 5) & 0x3F) +
                       ratio * ((bottomColor >> 5) & 0x3F));
        uint16_t b = ((1.0f - ratio) * (topColor & 0x1F) +
                       ratio * (bottomColor & 0x1F));

        uint16_t color = (r << 11) | (g << 5) | b;
        lcd_fill(0, y, lcddev.width, y + 1, color);
    }
}

// bo góc đơn giản
void lcd_round_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                    uint16_t radius, uint16_t color)
{
    lcd_fill(x + radius, y, x + w - radius, y + h, color);
    lcd_fill(x, y + radius, x + radius, y + h - radius, color);
    lcd_fill(x + w - radius, y + radius, x + w, y + h - radius, color);

    lcd_draw_circle(x + radius, y + radius, color, radius, 1);
    lcd_draw_circle(x + w - radius, y + radius, color, radius, 1);
    lcd_draw_circle(x + radius, y + h - radius, color, radius, 1);
    lcd_draw_circle(x + w - radius, y + h - radius, color, radius, 1);
}
